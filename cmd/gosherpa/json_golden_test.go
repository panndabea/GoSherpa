...
func TestMainAgentContextJSONGoldenFile(t *testing.T) {
	sourceFixtureRoot := filepath.Join("testdata", "json_project")
	fixtureRoot := t.TempDir()
	copyMainTestTree(t, sourceFixtureRoot, fixtureRoot)
	initMainTestGitRepository(t, fixtureRoot)
	...
	result := runMainTest(t, []string{"gosherpa", "--root", fixtureRoot, "agent", "context", "--base", "HEAD", "--json"})
	if result.ExitCode != exitSuccess {
		t.Fatalf("expected exit %d, got %d\nstderr:\n%s", exitSuccess, result.ExitCode, result.Stderr)
	}
	...
	actual := canonicalGoldenActualJSON(t, result.Stdout, fixtureRoot)
	expectedPath := filepath.Join("testdata", "golden-json", "agent-context.json")
	expectedBytes, err := os.ReadFile(expectedPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	if actual != expected {
		t.Fatalf("golden JSON mismatch for agent-context\nexpected:\n%s\nactual:\n%s", expected, actual)
	}
}
