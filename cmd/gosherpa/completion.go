...
var completionFlagDefinitions = map[string]completionFlag{
	...
	"--kind": {
		...
		TakesValue:  true,
		ValueName:   "kind",
		Values:      []string{"struct", "interface", "alias", "function", "method", "definition", "call", "read", "write", "type_usage", "field_access", "usage"},
	},
	"--scope": {
		Name:        "--scope",
...
func formatBashCompletion() string {
	...
	builder.WriteString("        refs) COMPREPLY=( $(compgen -W \"definition call read write type_usage field_access usage\" -- \"$cur\") ) ;;\n")
	builder.WriteString("      esac\n")
	builder.WriteString("      return\n")
	builder.WriteString("      ;;\n")
